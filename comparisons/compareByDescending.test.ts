...
Deno.test("returns a Collator", () => {
  const collator = compareByDescending<{ age: number }>("age");
  assertInstanceOf(collator, Collator);
});

Deno.test("string property key selector", async (t) => {
  const collator = compareByDescending<{ name: string }>("name");
...
Deno.test("number property key selector", async (t) => {
  const collator = compareByDescending<string[]>(3);
...
Deno.test("symbol property key selector", async (t) => {
  const id = Symbol("id");
  const collator = compareByDescending<{ [id]: bigint }>(id);
...
Deno.test("function selector", async (t) => {
  const collator = compareByDescending<{ nested: { value: number } }>(
    ({ nested: { value } }) => value,
  );
